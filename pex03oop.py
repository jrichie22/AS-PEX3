...
class DroneMission:
    ...
    def adjust_to_target_center(self, target_point, frame_write=None):
        ...
        # TODO: Calculate how far from center the target point (i.e. our person) is. (this looks done and I didn't touch it)
        dx = float(target_point[0]) - obj_track.FRAME_HORIZONTAL_CENTER
        dy = obj_track.FRAME_VERTICAL_CENTER - float(target_point[1])

        pixel_forgiveness = 5  # TODO: you decide what "good enough" is to consider centered on x or y axis...
        ...
        if self.mission_mode == MISSION_MODE_TARGET:
            ...
            if target_point is not None \
                ...
                if (self.direction_y == "C"
                    ...
                    # TODO: see below for setting your threshold... you decide what it should be (edited)
                    ...
                    if abs(dx) > pixel_distance_threshold:
                        # TODO: calculate a velocity for x-axis adjustment (edited)
                         xv = 0.6
                        # pass
                    else:
                        xv = 0.0
                    ...
                    if self.direction_y != "C":  # If we are not centered on y-axis...
                        if self.direction_y == "F":
                            ...
                            if frame_write is not None:
                                ...
                                #       or you can do a drone_lib.move_local as well. (done)
    ...
    def deliver_package(self, frame_write=None):
        ...
        if dist_to_object > 0:
            ...
            ground_dist = 0
            # TODO: use pex03_utils.get_ground_distance to get ground distance (done)
            ground_dist = pex03_utils.get_ground_distance(alt, dist_to_object)
            self.log_info(f"Ground distance: {ground_dist}.")

            # TODO: now, calculate new lat/long within 10 feet of objective (done)
            ...
            # TODO: Now, goto new location... (done)
            # HINT: drone_lib.goto_point(self.drone,.....)
            drone_lib.goto_point(self.drone, new_lat, new_lon, 1.0, alt)
            ...
            if self.virtual_mode:
                ...
                while self.drone.location.global_relative_frame.alt > alt_thresh:
                    ...
                    #       or you can do a drone_lib.move_local as well. (done)
                    drone_lib.small_move_down(self.drone)
                    # pass
            ...
            #  figure out how to open the latch to release the package (done)
    ...
    def conduct_mission(self):
        ...
        while self.drone.armed:
            ...
            # TODO: use obj_track to get current frame (done?)
            ...
            #  HINT: self.last_alt_pos = location.alt, etc... (done)
            ...
            if not self.object_identified:
                ...
                # TODO: set your confidence level here... (edited)
                ...
                if confidence is not None \
                    ...
                    # TODO: start tracking your object! (done?)
                ...
                # TODO: Continue to track our objective. (done?)
                #HINT: center, confidence, corner, radius, frm_display, bbox = obj_track.track_with_confirm
                center, confidence, corner, radius, frm_display, bbox = obj_track.track_with_confirm(frm_display)
